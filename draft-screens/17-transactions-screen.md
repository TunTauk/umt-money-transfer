...
[Search & Filter](07-search-filter.md) in full. Unlike the type-specific
...
design in [Data Model](02-data-model.md#design-decision-single-transaction-table):
...
- "Export CSV" button — implements the CSV/Excel export noted as an open
  item in [Search & Filter](07-search-filter.md#open): "cheap to add once
...
[Search & Filter](07-search-filter.md#free-text-search)). Deliberately
...
| Created by | Multi-select over `User` | **Admin/Owner only** — per [Search & Filter](07-search-filter.md#structured-filters), this is also the mechanism for reviewing a specific teller's activity (ties back to the pending-transaction monitoring note in [Transactions & Lifecycle](04-transactions-lifecycle.md#operational-implication)) |
...
| Party | Customer name for Send/Payout; "—" for internal types (Internal Transfer, Capital Deposit/Withdrawal have no customer identity, per [Data Model](02-data-model.md#transaction)) |
| Amount | Bold |
...
[Search & Filter](07-search-filter.md#sort) the two supported sorts are
...
- **Pagination** — spec calls for standard offset/cursor pagination
  ([Search & Filter](07-search-filter.md#implementation-notes)); not yet
